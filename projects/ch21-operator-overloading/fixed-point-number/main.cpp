...
public:
	...
	{
		// If either (or both) of the non-fractional and fractional part of the
		// number are negative, the number should be treated as negative
		...
		// and adjust base to account for the number of units removed from the
		// decimal
		m_base += m_decimal / 100;  // integer division
		m_decimal = m_decimal % 100; // integer remainder
	}
	
	// We'll delegate to the prior constructor so we don't have to duplicated 
	// the negative number and overflow handling logic
	...
	FixedPoint2 operator-() const {
		// Cast to double, make the double negative, then convert back to 
		// FixedPoint2
		return FixedPoint2 { -static_cast<double>(*this) };
	}
};

// This doesn't require access to the internals of the class, so it can be
